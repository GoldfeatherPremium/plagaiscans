...
import { FileCheck, Loader2, ArrowLeft, Eye, EyeOff, Home, UserCircle2 } from 'lucide-react';
...
export default function Auth() {
  ...
  const guestSpecial = searchParams.get('guest_special') === 'true';
              ...
              {authView === 'login' ? (
                <>
                  <form onSubmit={handleLogin} className="space-y-4">
                    <div className="space-y-2">
                      <Label htmlFor="email" className="font-semibold pl-2">{t('login.emailLabel')}</Label>
                      <Input
                        id="email"
                        type="email"
                        placeholder={t('login.emailPlaceholder')}
                    ...
                    <div className="space-y-2">
                      <Label htmlFor="password" className="font-semibold pl-2">{t('login.passwordLabel')}</Label>
                      <div className="relative">
                        <Input
                          id="password"
                          ...
                          className="pr-12 h-12 rounded-full px-5"
                        />
                        <Button
                          type="button"
                          variant="ghost"
                          size="icon"
                          className="absolute right-1 top-1/2 -translate-y-1/2 h-10 w-10 rounded-full hover:bg-transparent"
                          onClick={() => setShowLoginPassword(!showLoginPassword)}
                        >
                          {showLoginPassword ? <EyeOff className="h-4 w-4 text-muted-foreground" /> : <Eye className="h-4 w-4 text-muted-foreground" />}
                        </Button>
                      </div>
                      {errors.password && <p className="text-sm text-destructive pl-2">{errors.password}</p>}
                    </div>
                    <div className="flex items-center justify-between pl-2">
                      ...
                      <Button
                        type="button"
                        variant="link"
                        className="px-0 text-sm text-primary italic"
                        onClick={() => setShowForgotPassword(true)}
                      >
                        {t('login.forgotPassword')}
                      </Button>
                    </div>
                    <Button type="submit" className="w-full h-12 rounded-full text-base font-semibold shadow-md" disabled={loading}>
                      {loading && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
                ...
                <>
                  <form onSubmit={handleSignup} className="space-y-4">
                    ...
                    <div className="space-y-2">
                      <Label htmlFor="confirmPassword" className="font-semibold pl-2">{t('signup.confirmPasswordLabel')}</Label>
                      <div className="relative">
                        <Input
                          id="confirmPassword"
                      ...
                      {errors.confirmPassword && <p className="text-sm text-destructive pl-2">{errors.confirmPassword}</p>}
                    </div>
                    <div className="space-y-2">
                      <Label htmlFor="referralCode" className="font-semibold pl-2">Referral Code (Optional)</Label>
                      <Input
                        id="referralCode"
                        type="text"
                        ...
                        className="h-12 rounded-full px-5"
                      />
                      <p className="text-xs text-muted-foreground pl-2">Have a friend's referral code? Enter it to earn bonus credits!</p>
                    </div>
