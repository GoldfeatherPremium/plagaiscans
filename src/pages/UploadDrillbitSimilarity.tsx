...
const ALLOWED_EXT = /\.(pdf|docx|txt|rtf|odt|xlsx|pptx|html|hwp)$/i;

const UploadDrillbitSimilarity: React.FC = () => {
  ...
  const balance =
    (profile as { drillbit_similarity_credit_balance?: number } | null)
      ?.drillbit_similarity_credit_balance ?? 0;
  ...
  const addFiles = (incoming: File[]) => {
    ...
    if (valid.length > room) {
      toast.error(`You can only upload ${room} more file(s) with your current credits.`);
    }
  ...
  const handleSubmit = async () => {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      try {
        await uploadSimilarityDocument(files[i], undefined, 'drillbit_similarity_only');
        success++;
      } catch (err: any) {
        ...
        <div className="max-w-2xl mx-auto p-6">
          <Card>
            <CardContent className="p-8 text-center space-y-4">
              ...
              <h2 className="text-2xl font-bold">No Drillbit similarity credits</h2>
              <p className="text-muted-foreground">
                You need at least 1 Drillbit similarity credit to upload here.
              </p>
        ...
        <div className="flex items-start justify-between">
          <div>
            <h1 className="text-3xl font-bold flex items-center gap-2">
              <FileSearch className="h-7 w-7 text-orange-500" />
              Drillbit — Plagiarism only
            </h1>
            <p className="text-muted-foreground mt-1">
              Upload documents for Drillbit similarity analysis. No AI detection.
            </p>
          </div>
          ...
          <CardContent className="p-6 space-y-4">
            ...
            >
              ...
              <p className="text-sm text-muted-foreground mt-1">
                .pdf, .docx, .txt, .rtf, .odt, .xlsx, .pptx, .html, .hwp
              </p>
              <input
                ref={inputRef}
                type="file"
                multiple
                accept=".pdf,.docx,.txt,.rtf,.odt,.xlsx,.pptx,.html,.hwp"
                className="hidden"
...
export default UploadDrillbitSimilarity;
