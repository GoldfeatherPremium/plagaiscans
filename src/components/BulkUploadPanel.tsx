        ...
        <CardContent>
          ...
          >
            ...
            <div className="flex flex-col items-center gap-2">
              ...
              <div>
                <p className="font-medium text-sm">Drop PDF files or ZIP archives here</p>
          ...
          {files.length > 0 && (
            <div className="mt-4">
              ...
              <ScrollArea className={compact ? "h-[150px] border rounded-lg" : "h-[200px] border rounded-lg"}>
              ...
              {files.length > 0 && pendingDocuments.length > 0 && !processing && (
                <div className="mt-4 grid grid-cols-3 gap-2">
                  ...
                  <div className="text-center p-2 bg-yellow-500/10 rounded-lg border border-yellow-200">
                    <p className="text-lg font-bold text-yellow-600">{matchStats.partial}</p>
                    <p className="text-xs text-muted-foreground">Partial</p>
                  </div>
                  <div className="text-center p-2 bg-red-500/10 rounded-lg border border-red-200">
                    <p className="text-lg font-bold text-red-600">{matchStats.none}</p>
          ...
          <CardContent>
            ...
            <div className="grid grid-cols-5 gap-2 mb-4">
              <div className="text-center p-2 bg-muted rounded-lg">
                <p className="text-lg font-bold">{processingResult.stats.totalReports}</p>
                <p className="text-xs text-muted-foreground">Total</p>
              </div>
              <div className="text-center p-2 bg-green-500/10 rounded-lg">
                <p className="text-lg font-bold text-green-600">{processingResult.stats.mappedCount}</p>
                <p className="text-xs text-muted-foreground">Mapped</p>
              </div>
              <div className="text-center p-2 bg-blue-500/10 rounded-lg">
                <p className="text-lg font-bold text-blue-600">{processingResult.stats.completedCount}</p>
              ...
              <div className="text-center p-2 bg-orange-500/10 rounded-lg">
                <p className="text-lg font-bold text-orange-600">{processingResult.stats.needsReviewCount}</p>
            ...
            {processingResult.mapped.length > 0 && (
              <div className="mb-4">
                <h4 className="text-sm font-medium mb-2 flex items-center gap-2">
                  ...
                  Mapped ({processingResult.mapped.length})
                </h4>
                <ScrollArea className="h-[100px] border rounded-lg">
                  <div className="p-2 space-y-1">
                    {processingResult.mapped.map((item, index) => (
                      <div key={index} className="flex items-center justify-between p-1.5 bg-green-500/5 rounded">
                        <span className="text-xs truncate flex-1">{item.fileName}</span>
                        <div className="flex items-center gap-1">
                          <Badge variant={item.reportType === 'similarity' ? 'default' : 'secondary'} className="text-xs">
                            {item.reportType}
                          </Badge>
                ...
                <ScrollArea className="h-[100px] border rounded-lg">
                  <div className="p-2 space-y-1">
                    {processingResult.unmatched.map((item, index) => (
                      <div key={index} className="flex items-center justify-between p-1.5 bg-yellow-500/5 rounded">
                        <div className="min-w-0 flex-1">
            ...
            {processingResult.completedDocuments.length > 0 && (
              <div className="p-3 bg-green-500/10 rounded-lg">
                <p className="text-sm font-medium text-green-700">
                  ✓ {processingResult.completedDocuments.length} document(s) completed. Customers notified.
                </p>
              </div>
            )}
          </CardContent>
