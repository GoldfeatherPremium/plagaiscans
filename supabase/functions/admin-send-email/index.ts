...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    const { 
      type, targetAudience, specificUserIds, 
      subject, title, message, ctaText, ctaUrl, logId 
    ...
    if (targetAudience === 'specific' && specificUserIds?.length) {
      let query = supabase.from("profiles").select("id, email").in("id", specificUserIds);
      if (isPromotional) query = query.or('email_unsubscribed.is.null,email_unsubscribed.eq.false');
      ...
      if (customerRoles?.length) {
        let query = supabase.from("profiles").select("id, email").in("id", customerRoles.map(r => r.user_id));
        if (isPromotional) query = query.or('email_unsubscribed.is.null,email_unsubscribed.eq.false');
        const { data: profiles } = await query;
        recipients = profiles?.map(p => ({ id: p.id, email: p.email })) || [];
      }
      ...
      if (staffRoles?.length) {
        let query = supabase.from("profiles").select("id, email").in("id", staffRoles.map(r => r.user_id));
        if (isPromotional) query = query.or('email_unsubscribed.is.null,email_unsubscribed.eq.false');
        const { data: profiles } = await query;
        recipients = profiles?.map(p => ({ id: p.id, email: p.email })) || [];
      }
      ...
      if (adminRoles?.length) {
        let query = supabase.from("profiles").select("id, email").in("id", adminRoles.map(r => r.user_id));
        if (isPromotional) query = query.or('email_unsubscribed.is.null,email_unsubscribed.eq.false');
        const { data: profiles } = await query;
        recipients = profiles?.map(p => ({ id: p.id, email: p.email })) || [];
      }
    ...
    const typeIcons: Record<string, string> = {
      announcement: '📢', payment_reminder: '💳', document_status: '📄',
                ...
                <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width: 600px; background-color: white; border-radius: 12px; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1);">
                  <tr>
                    <td style="padding: 40px;">
                      ...
                      ${ctaText && ctaUrl ? `
                      <div style="text-align: center; margin-bottom: 30px;">
                        <a href="${ctaUrl}" style="display: inline-block; background: linear-gradient(135deg, #3b82f6 0%, #8b5cf6 100%); color: white; text-decoration: none; padding: 14px 28px; border-radius: 8px; font-weight: 600;">${ctaText}</a>
                      </div>` : ''}
                      <div style="margin-top: 40px; padding-top: 20px; border-top: 1px solid #e5e7eb;">
                        <p style="color: #6b7280; text-align: center; margin: 0 0 10px 0; font-size: 13px;">
                          Regards,<br><strong>Plagaiscans Support Team</strong>
                        </p>
                        <p style="color: #9ca3af; text-align: center; margin: 0; font-size: 12px;">
                          ...
                          ${isPromotional ? `<br><br><span style="color: #71717a;">Don't want to receive promotional emails?</span><br><a href="${unsubscribeUrl}" style="color: #9ca3af; text-decoration: underline; font-size: 11px;">Unsubscribe</a>` : ''}
                        </p>
                      </div>
                    </td>
                  </tr>
    ...
    return new Response(
      JSON.stringify({ success: true, sent: successCount, failed: failedCount, total: recipients.length }),
