...
const logStep = (step: string, details?: any) => {
  ...
  console.log(`[CREATE-STRIPE-EMBEDDED-CHECKOUT] ${step}${detailsStr}`);
};
...
function validateInput(data: any): { valid: boolean; error?: string } {
  const { credits, amount, creditType } = data;
...
serve(async (req) => {
  ...
  try {
    ...
    const { credits, amount, creditType = "full" } = requestBody;
    logStep("Request body parsed", { credits, amount, creditType });
